...
class TestReport(TestWornBase):
    def test_init(self):
      ...
      with patch.object(Project, 'last', return_value=LogProject(p1.id, p1.name, 'started', time_traveled(since=when, seconds=10))) as last:
        r = Report([])
      ...
      with patch.object(Project, 'last', return_value=LogProject(p1.id, p1.name, 'started', when)) as last:
    ...
    def test_init2(self):
      when = now()
      p1 = LogProject(uuid4(), 'Ima sneeky thing', 'started', time_traveled(since=when, seconds=92))
      p2 = LogProject(p1.id,   p1.name,            'stopped', time_traveled(since=when, seconds=33))
    ...
    def test_mail(self):
      _uuid = uuid4()
      with patch.object(Project, 'last', return_value=LogProject(17, 'Bye', 'started', now())) as last:
    ...
    def test_sorted_data(self):
      _uuid = uuid4()
      _uuid2 = uuid4()
      with patch.object(Project, 'last', return_value=LogProject(13, 'xyz', 'started', now())) as last:
    ...
    def test_post_with_noop_doesnt_post(self):
      ...
      when = now()
      with patch('builtins.print') as mock_debug:
        with patch.multiple(Project, cache=DEFAULT, last=DEFAULT, all=DEFAULT) as whos_line:
    ...
    def test_format(self):
      with patch.object(Project, 'last', return_value=LogProject(3, 'aw', 'started', now())) as last:
        r = Report({})
        ...
        with patch('builtins.super') as passthrough_fmt:
          self.assertIn('<lib.report.Report object at 0x7', f'{r!r}')
    ...
    def test_format_csv(self):
      with patch.object(Project, 'last', return_value=LogProject(9, 'BMW', 'started', now())) as last:
        r = Report({})
        ...
        _data = (
          ...
          LogProject(11, 'autobus', 'stopped', when=_now) # < so we can test that 0 values are considered
        )
    ...
    def test_format_time(self):
      with patch.object(Project, 'last', return_value=LogProject(5, 'Audi', 'started', now())) as last:
        r = Report({})
        self.assertEqual(last.call_count, 1)
